...
folder = "Metropolis/"
data = pd.read_csv(folder + "Metropolis_x.txt", header=None)
x = np.asarray(data[0])
data = pd.read_csv(folder + "Metropolis_Prop.txt", header=None, sep=' ')
P_ave = np.asarray(data[0])
P_err = np.asarray(data[1])
data = pd.read_csv(folder + "Metropolis_Sub.txt", header=None, sep=' ')
...
plt.title("Amplitude of the Propagator", fontsize=16)
...
plt.title("Amplitude of the Subpropagator", fontsize=16)
plt.xlabel("x", fontsize=14)
