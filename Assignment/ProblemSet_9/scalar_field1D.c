...
void metro(int mtimes, double phimax, double *phi, double *newphi, int N, double a, double cons, double *accept, gsl_rng *rng);
void hmc(int hmcs, double dt, int N, double a, double cons, double *phi, double *newphi, double *ph, double *accept, gsl_rng *rng);
void ph_u(double dt, int N, double a, double cons, double *ph, double *newphi);
...
int main(){
	...
	double x, y;			// end-points of the propagator.
	...
	for(int i = 0; i <= sweeps; i = i+1){
		// Algorithm
		if(alg == 0){
			hmc(hmcs, dt, N, a, cons, phi, newphi, ph, &accept, rng);
		}
		else{
			metro(mtimes, phimax, phi, newphi, N, a, cons, &accept, rng);
		}
	...
	*accept = 0.0;
	for(int j = 0; j < N; j = j+1){
		...
		for(int k = 1; k <= mtimes; k = k+1){
			po = phi[j];
			so = -po * ps + cons * pow(po, 2);
			pn = (gsl_rng_uniform(rng) - 0.5) * phimax;
			sn = -pn * ps + cons * pow(pn, 2);
			ds = (sn - so) / a;
...
void hmc(int hmcs, double dt, int N, double a, double cons, double *phi, double *newphi, double *ph, double *accept, gsl_rng *rng){
	...
	*accept = 0.0;
	...
	for(int j = 0; j < N; j = j+1){
		...
		fw = (j + 1) % N;
		ps = phi[fw];
		oldh = oldh + (-po * ps + cons * pow(po, 2)) / a + 0.5 * pow(ph[j], 2);
	}
	ph_u(dt/2, N, a, cons, ph, newphi);
	for(int i = 1; i < hmcs; i = i+1){
		phi_u(dt, N, ph, newphi);
		ph_u(dt, N, a, cons, ph, newphi);
	}
	phi_u(dt, N, ph, newphi);
	ph_u(dt/2, N, a, cons, ph, newphi);
	...
	for(int j = 0; j < N; j = j+1){
		...
		fw = (j + 1) % N;
		ps = newphi[fw];
		newh = newh + (-po * ps + cons * pow(po, 2)) / a + 0.5 * pow(ph[j], 2);
	}
...
void ph_u(double dt, int N, double a, double cons, double *ph, double *newphi){
	...
	for(int j = 0; j < N; j = j+1){
		fw = (j + 1) % N;
		bw = (j - 1 + N) % N;
		ph[j] = ph[j] + dt * (newphi[fw] + newphi[bw] - cons * 2.0 * newphi[j]) / a;
	}
}
